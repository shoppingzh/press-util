...
export function readdirByOrders(path: string): Dirent[] {
  ...
  const orderItemIndex = list.findIndex(o => o.isFile() && o.name.toUpperCase() === ORDER_FILE_NAME)
  if (orderItemIndex < 0) return list
  const orderItem = list[orderItemIndex]
  list.splice(orderItemIndex, 1)
  const orderContent = readFileSync(join(path, orderItem.name), { encoding: 'utf-8' })
...
export function listFiles(path: string): File[] {
  return readdirByOrders(path).filter(o => o.isFile()).map(o => ({
    dirent: o,
